  ...
  <main class="mx-32 my-10 font-poppins">
    ...
    <section
      ...
      <div v-for="category in categories" :key="category" class="relative">
        <!-- Emit ✅ -->
        <CategoryCard :categories="category" @add-note=redirectToCategory(category)
          @remove-category=removeCategory(category) />
        <!-- Dynamic routing ✅ -->
        <router-link :to="{ name: 'category', params: { categoryName: category } }">
        </router-link>
      </div>
    </section>
  </main>
...
export default {
  ...
  methods: {
    ...
    categoriesAPI() { //forbidden gtaw knp awikwok
